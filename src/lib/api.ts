...
function getEffectiveApiBase(): string {
  ...
  if (typeof window !== 'undefined') {
    ...
    // If browsing on a remote domain/preview URL and envBase points to localhost, ignore it to prevent "Failed to fetch"
    if (!isLocalhost && (/localhost/i.test(envBase) || /127\.0\.0\.1/i.test(envBase))) {
      return '';
    }
  }
