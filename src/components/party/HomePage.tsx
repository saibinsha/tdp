...
const HomePage: React.FC = () => {
  ...
  const [newsLeaderFilter, setNewsLeaderFilter] = useState<string>('all');

  const DEFAULT_LEADERS = [
    {
      slug: 'cbn',
      name: 'N. Chandrababu Naidu',
      role: 'Party President & Chief Minister',
    ...
    },
    {
      slug: 'lokesh',
      name: 'Nara Lokesh',
      role: 'National General Secretary & IT Minister',
      category: 'state_leadership',
  ...
  const [leadersList, setLeadersList] = useState<any[]>(DEFAULT_LEADERS);

  useEffect(() => {
    let active = true;
    api.request<{ ok: true; items: any[] }>('/api/leaders?activeOnly=true', 'GET')
      .then((res) => {
        if (!active) return;
        if (Array.isArray(res?.items) && res.items.length > 0) {
          setLeadersList(res.items);
        }
      })
      .catch(() => {});
    return () => {
  ...
  const newsTrackedLeaders = useMemo(() => {
    return leadersList.filter((l) => l.trackInNews !== false);
  }, [leadersList]);
    ...
    <div>
      ...
      <section className="bg-gray-50 py-16">
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            <button
              onClick={() => {
                setNewsLeaderFilter('all');
                setNewsExpanded(false);
              }}
              className={`px-4 py-2 rounded-full text-sm font-bold border transition-colors ${
                newsLeaderFilter === 'all'
            ...
            {newsTrackedLeaders.map((ldr) => {
              const filterId = ldr.slug || ldr.name;
              const active = newsLeaderFilter === filterId;
              return (
                <button
                  key={ldr._id || ldr.slug || ldr.name}
                  onClick={() => {
                    setNewsLeaderFilter(filterId);
                    setNewsExpanded(false);
                  }}
                  className={`px-4 py-2 rounded-full text-sm font-bold border transition-colors flex items-center gap-1.5 ${
                    active
                      ? 'bg-blue-600 text-white border-blue-600'
                      : 'bg-white text-gray-700 border-gray-200 hover:bg-gray-50'
                  }`}
                >
                  <span>{ldr.name.replace(/^N\.\s*/, '').split(' ')[0] || ldr.name}</span>
                </button>
              );
            })}
          </div>
      ...
      <section className="max-w-7xl mx-auto px-4 py-16">
        <div className="text-center mb-12">
          ...
          <p className="text-gray-500">Dedicated leaders working for the welfare and prosperity of Andhra Pradesh</p>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {leadersList.map((leader, i) => (
            <div key={leader._id || leader.slug || i} className="text-center group bg-white p-5 rounded-2xl border border-gray-100 shadow-sm hover:border-yellow-400 hover:shadow-md transition-all">
              <div className="relative w-28 h-28 mx-auto mb-4 rounded-full overflow-hidden border-4 border-yellow-400 shadow-md group-hover:border-blue-600 transition-colors bg-yellow-100 flex items-center justify-center">
                {leader.photoUrl || leader.image ? (
                  <img src={leader.photoUrl || leader.image} alt={leader.name} className="w-full h-full object-cover" />
                ) : (
              ...
              <p className="text-xs text-blue-700 font-semibold mt-1">{leader.role}</p>
              {(leader.constituency || leader.district) && (
                <p className="text-[11px] text-gray-400 mt-0.5">
                  {[leader.constituency, leader.district].filter(Boolean).join(', ')}
                </p>
              )}
            </div>
          ))}
