...
function inferResourceTypeFromUrl(url: string): 'image' | 'video' | 'raw' {
  ...
  if (u.match(/\.(mp4|webm|mov|m4v|avi|ogv)(\?|#|$)/)) return 'video';
...
const BlogsPage: React.FC = () => {
  ...
  const handleAddMediaLink = () => {
    const raw = mediaLink.trim();
    if (!raw) return;
    const items = raw.split(/[\n,]+/).map((s) => s.trim()).filter(Boolean);
    const added: BlogMedia[] = [];
    for (const item of items) {
      const normalized = normalizeMediaUrl(item);
      if (normalized) {
        const resourceType = inferResourceTypeFromUrl(normalized);
        added.push({ url: normalized, resourceType });
      }
    }
    if (added.length > 0) {
      setDraftMedia((prev) => [...prev, ...added]);
      setMediaLink('');
    }
  };
            ...
            {draftMedia.length > 0 && (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {draftMedia.map((m, idx) => (
                  <div key={idx} className="relative border border-gray-200 rounded-xl overflow-hidden bg-gray-50 aspect-video flex items-center justify-center">
          ...
          <div key={blog._id} className="bg-white rounded-xl border border-gray-100 overflow-hidden shadow-sm hover:shadow-md transition-shadow">
            {blog.media?.[0]?.url && (
              <div className="relative overflow-hidden bg-gray-100 flex items-center justify-center">
                ...
                {blog.media.length > 1 && (
                  <span className="absolute bottom-2 right-2 px-2 py-0.5 bg-black/70 text-white rounded-md text-[11px] font-bold">
                    +{blog.media.length - 1} more
                  </span>
                )}
              </div>
            )}
