...
const GroupsPage: React.FC = () => {
  ...
  const startLocalMedia = async (kind: 'audio' | 'video') => {
    ...
    if (kind === 'video') {
      try {
        ...
        console.warn('[GroupCall] Detailed video constraints failed, trying basic video:', videoErr);
        try {
          stream = await navigator.mediaDevices.getUserMedia({ audio: true, video: true });
        } catch (basicErr) {
          console.warn('[GroupCall] Basic video getUserMedia failed, falling back to audio:', basicErr);
        }
      }
    }
    ...
    if (!stream) {
      try {
        ...
        console.warn('[GroupCall] Audio getUserMedia failed, using dummy stream:', audioErr);
  ...
  }, [callOpen, callKind, callStatus, remoteStreams]);
