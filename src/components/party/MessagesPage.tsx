...
const MessagesPage: React.FC = () => {
  ...
  const callOtherRef = useRef<{ _id: string; name: string } | null>(null);
  const callKindRef = useRef<'audio' | 'video'>('audio');
  const callStatusRef = useRef<'calling' | 'ringing' | 'connecting' | 'connected' | 'ended'>('calling');
  ...
  useEffect(() => {
    callOtherRef.current = callOther;
  }, [callOther]);
  ...
  const cleanupCall = () => {
    ...
    try {
      if ((window as any).Android?.endCall) {
        (window as any).Android.endCall();
      }
    } catch {
      // ignore
    }
  ...
  const ensurePc = (sock: any, id: string, targetUserId?: string) => {
        ...
        try {
          if ((window as any).Android) {
            if (callKindRef.current === 'video') {
              (window as any).Android.startCall?.();
            } else {
              (window as any).Android.startAudioCall?.();
            }
          }
        } catch {
          // ignore
    ...
    pc.ontrack = (ev) => {
      console.log('[WebRTC] ontrack received:', ev.track.kind, ev.streams);
      let r = remoteStreamRef.current;
      if (!r) {
        r = new MediaStream();
        remoteStreamRef.current = r;
      }
      for (const track of ev.streams?.[0]?.getTracks?.() || []) {
        if (!r.getTracks().some((t) => t.id === track.id)) r.addTrack(track);
      }
      if (ev.track && !r.getTracks().some((t) => t.id === ev.track.id)) {
        r.addTrack(ev.track);
      }
      if (remoteVideoRef.current) {
        if (remoteVideoRef.current.srcObject !== r) {
          remoteVideoRef.current.srcObject = r;
        }
        remoteVideoRef.current.play().catch(() => {});
      }
      if (remoteAudioRef.current) {
        if (remoteAudioRef.current.srcObject !== r) {
          remoteAudioRef.current.srcObject = r;
        }
        remoteAudioRef.current.play().catch(() => {});
      }
    };
    ...
    pc.onicecandidate = (ev) => {
      ...
      const otherId = targetUserId || callOtherRef.current?._id || callOther?._id;
      if (!otherId) return;
      sock.emit('call:ice', {
        callId: id,
  ...
  const startLocalMedia = async (kind: 'audio' | 'video') => {
    ...
    if (kind === 'video') {
      try {
        ...
        console.warn('[Call] Detailed video constraints failed, trying basic video:', videoErr);
        try {
          stream = await navigator.mediaDevices.getUserMedia({ audio: true, video: true });
        } catch (basicErr) {
          console.warn('[Call] Basic video getUserMedia failed, falling back to audio:', basicErr);
        }
      }
    }
    ...
    if (!stream) {
      try {
        ...
        console.warn('[Call] Audio getUserMedia failed, trying basic audio:', audioErr);
        try {
          stream = await navigator.mediaDevices.getUserMedia({ audio: true });
        } catch (fallbackErr) {
          console.warn('[Call] Fallback audio failed, using empty stream:', fallbackErr);
  ...
  const hangup = () => {
    ...
    const otherId = String(callOtherRef.current?._id || callOther?._id || '');
    if (s && callIdRef.current) {
      s.emit('call:hangup', { callId: callIdRef.current, toUserId: otherId });
    }
    cleanupCall();
  };
  ...
  const placeCall = async (kind: 'audio' | 'video', opts?: { autoAnswer?: boolean }) => {
    ...
    callOtherRef.current = { _id: otherId, name: otherName };
    setCallIncoming(false);
    setCallOpen(true);
    ...
    const pc = ensurePc(sock, id, otherId);
    ...
    try {
      ...
      if (kind === 'video' && !hasVideo) pc.addTransceiver('video', { direction: 'sendrecv' });
    } catch {
      // ignore
    }
  ...
  const acceptIncoming = async (incomingCallId?: string) => {
    const id = String(incomingCallId || callIdRef.current || callId || '').trim();
    if (!id) return;
    ...
    const otherId = String(callOtherRef.current?._id || callOther?._id || '');
    const kind = callKindRef.current || callKind;

    // Notify Android native layer if running in WebView
    try {
      if ((window as any).Android) {
        if (kind === 'video') {
          (window as any).Android.startCall?.();
    ...
    try {
      ...
      sock.emit('call:accept', { callId: id, toUserId: otherId });
    } catch (e) {
  ...
  const rejectIncoming = () => {
    ...
    if (!callSocketRef.current) callSocketRef.current = sock;
    const otherId = String(callOtherRef.current?._id || callOther?._id || '');
    const cid = String(callIdRef.current || callId || '');
    if (cid) sock.emit('call:reject', { callId: cid, toUserId: otherId });
    cleanupCall();
  };
  ...
  useEffect(() => {
    ...
    try {
      ...
      const cid = String(parsed.callId);
      const kind = parsed.kind === 'video' ? 'video' : 'audio';
      const otherUser = { _id: String(parsed.fromUserId), name: String(parsed.fromName || 'Member') };
  ...
  useEffect(() => {
    ...
    const onIncoming = (payload: any) => {
      ...
      const otherUser = { _id: fromId, name: String(payload?.from?.name || 'Member') };
    ...
    const onAccepted = async (payload: any) => {
      ...
      try {
        ...
        const offer = await pc.createOffer({
          ...
          offerToReceiveVideo: callKindRef.current === 'video' || callKind === 'video',
        });
        await pc.setLocalDescription(offer);
        sock.emit('call:offer', { callId: id, sdp: offer, toUserId: otherId });
      } catch (e) {
        console.error('[Call] onAccepted createOffer error:', e);
      }
    };
    ...
    const onOffer = async (payload: any) => {
      ...
      const otherId = String(payload?.fromUserId || callOtherRef.current?._id || callOther?._id || '');
      const pc = ensurePc(sock, id, otherId);
      ...
      try {
        ...
        sock.emit('call:answer', { callId: id, sdp: ans, toUserId: otherId });
      } catch (e) {
        console.error('[Call] onOffer error:', e);
      }
    };
    ...
    const onAnswer = async (payload: any) => {
      ...
      try {
        ...
        console.error('[Call] onAnswer error:', e);
      }
    };
