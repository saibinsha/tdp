...
function registerChatHandlers(io, socket) {
  ...
  socket.on('call:accept', async ({ callId, toUserId }, cb) => {
    const cid = String(callId || '');
    const call = activeCalls.get(cid);
    const targetUserId = toUserId ? String(toUserId) : (call ? String(call.callerId) : null);
    ...
    cb && cb({ ok: false, message: 'Caller not found' });
  });

  socket.on('call:reject', async ({ callId, toUserId }, cb) => {
    const cid = String(callId || '');
    const call = activeCalls.get(cid);
    const targetUserId = toUserId ? String(toUserId) : (call ? String(call.callerId) : null);
    if (targetUserId) {
      io.to(`user:${targetUserId}`).emit('call:rejected', { callId: cid });
    }
    activeCalls.delete(cid);
    cb && cb({ ok: true });
  });

  function otherParty(call, explicitToUserId) {
    if (explicitToUserId) return String(explicitToUserId);
    if (!call) return null;
  ...
  socket.on('call:offer', ({ callId, sdp, toUserId }, cb) => {
    const cid = String(callId || '');
    const call = activeCalls.get(cid);
    const other = otherParty(call, toUserId);
    if (!other) return cb && cb({ ok: false, message: 'Target not found' });
    io.to(`user:${other}`).emit('call:offer', {
      callId: cid,
      sdp,
      fromUserId: String(socket.user._id),
      kind: call ? call.kind : undefined,
    });
    cb && cb({ ok: true });
  });

  socket.on('call:answer', ({ callId, sdp, toUserId }, cb) => {
    const cid = String(callId || '');
    const call = activeCalls.get(cid);
    const other = otherParty(call, toUserId);
    if (!other) return cb && cb({ ok: false, message: 'Target not found' });
    io.to(`user:${other}`).emit('call:answer', {
      callId: cid,
      sdp,
      fromUserId: String(socket.user._id),
    });
  ...
  socket.on('call:ice', ({ callId, candidate, toUserId }, cb) => {
    const cid = String(callId || '');
    const call = activeCalls.get(cid);
    const other = otherParty(call, toUserId);
    if (!other) return cb && cb({ ok: false, message: 'Target not found' });
    io.to(`user:${other}`).emit('call:ice', {
      callId: cid,
      candidate,
      fromUserId: String(socket.user._id),
    });
  ...
  socket.on('call:hangup', ({ callId, toUserId }, cb) => {
    const cid = String(callId || '');
    const call = activeCalls.get(cid);
    const other = otherParty(call, toUserId);
    if (other) {
      io.to(`user:${other}`).emit('call:hangup', {
        callId: cid,
        fromUserId: String(socket.user._id),
      });
    }
