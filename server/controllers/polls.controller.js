...
const getMyVotes = asyncHandler(async (req, res) => {
  const votes = await PollVote.find({ user: req.user._id }).select('poll optionId createdAt');
  const votedMap = {};
  for (const v of votes) {
    votedMap[String(v.poll)] = String(v.optionId);
  }
  res.json({ ok: true, votedMap, votes });
});

module.exports = { createPoll, listPolls, votePoll, getResults, deletePoll, getMyVotes };
