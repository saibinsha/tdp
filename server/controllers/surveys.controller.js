...
const getMyResponses = asyncHandler(async (req, res) => {
  const list = await SurveyResponse.find({ user: req.user._id }).select('survey answers createdAt');
  const submittedMap = {};
  for (const r of list) {
    submittedMap[String(r.survey)] = true;
  }
  res.json({ ok: true, submittedMap, responses: list });
});

module.exports = { createSurvey, listSurveys, getSurvey, submitResponse, analytics, deleteSurvey, getMyResponses };
